class VehiclePerformedCheck < ApplicationRecord
  ...
  def create_notification(user)
    ...
    unless self.performed == 0 or self.performed == 1 or self.performed == 3
      ...
      ewc.close
      opcode = VehiclePerformedCheck.get_ms_client.execute("select nominativo from autisti where idautista = "+vehicle.last_driver.mssql_references.last.remote_object_id.to_s).first['nominativo'].gsub("'","''") unless vehicle.last_driver.nil?
