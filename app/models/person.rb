class Person < ApplicationRecord
  ...
  scope :find_by_complete_name,->(name) { where("concat_ws(' ',surname,name) = ?", name) }
  scope :filter, ->(name) { where("name like ? or surname like ? or mdc_user like ? or ('mdc' like ? and mdc_user is not null)", "%#{name}%", "%#{name}%", "%#{name}%", "%#{name}%").order(:surname) }
