class ExternalVehicle < ApplicationRecord
  ...
  scope :filter, ->(search) { where("plate like '%#{search.to_s.tr(' ','%').tr('*','%')}%' or vehicle_type_id in (select id from vehicle_types where name like '%#{search.to_s.tr(' ','%').tr('*','%')}%') "\
                "or vehicle_typology_id in (select id from vehicle_typologies where name like '%#{search.to_s.tr(' ','%').tr('*','%')}%') "\
                "or owner_id in (select id from companies where name like '%#{search.to_s.tr(' ','%').tr('*','%')}%')").distinct }
