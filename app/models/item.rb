class Item < ApplicationRecord
  ...
  scope :tyres, -> { joins(:article).joins('left join article_categorizations on articles.id = article_categorizations.article_id').joins('left join article_categories on article_categorizations.category_id = article_categories.id').where('article_categories.name like \'%gomme%\'') }
  scope :notyres, -> { joins(:article).joins('left join article_categorizations on articles.id = article_categorizations.article_id').joins('left join article_categories on article_categorizations.category_id = article_categories.id').where('article_categories.name not like \'%gomme%\'') }
